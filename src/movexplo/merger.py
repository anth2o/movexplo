...
if __name__ == "__main__":
    ...
    parser.add_argument('--md5_file', type=str, help='the json file where the md5 are present')
    parser.add_argument('--enriched_file', type=str, help='the file with additional data')
    ...
    for enriched_line in enriched_lines:
        md5_list = [
            md5_line["md5"] for md5_line in md5_lines if md5_line["name"] == enriched_line["name"]
        ]
        if len(md5_list):
